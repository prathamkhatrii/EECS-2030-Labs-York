...
public final class Lab1 {
	 ...
	 *      method return an int. This means the return value at most requires 4
	 *      bytes and no overflow would happen.
	 * 
	...
	public static String makeString(char first, int n) {
		...
		if (n == 0) {
			...
			str += first + makeString(first, n - 1);
			return str;
		}
	 ...
	 * Before the asterisk(s) you should add less-than characters ("<"). After the
	 * asterisk(s) you should add greater-than characters (">").
	 * 
	 * @param n is the total number of strings returned by the method
	 * @return returns a string made of 1 or 2 stars between a number of "<" and ">"
	 *         characters. The string length should be equal to n
	 ...
	 *         <code> open </code> and <code> close </code>.
	 * @pre The given str contains only one <code> open </code> and one
	 *      <code> close </code> character.
	 */
	public static String getSubstring(String str, char open, char close) {
		int indexOpen = str.indexOf(open);
		int indexClose = str.indexOf(close);

		if (indexOpen == indexClose - 1) {
			return "";
		}

		return str.charAt(indexOpen + 1) + getSubstring(str.substring(indexOpen + 1), str.charAt(indexOpen + 1), close);
